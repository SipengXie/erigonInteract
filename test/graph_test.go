...
func TestMakeGraph(t *testing.T) {
	...
	st := time.Now()
	graph := OldGenerateUndiGraph(txs, predictRwSets)
	fmt.Println("Old graph time", time.Since(st))

	fmt.Println("Graph Vertices", len(graph.Vertices))
	sum := 0
	for _, v := range graph.AdjacencyMap {
		sum += len(v)
	}
	fmt.Println("Graph Edges", sum)

	st = time.Now()
	newGraph := utils.GenerateUndiGraph(len(txs), rwAccessedBy)
	fmt.Println("New graph time", time.Since(st))

	fmt.Println("New Graph Vertices", len(newGraph.Vertices))
	sum = 0
	for _, v := range newGraph.AdjacencyMap {
		sum += len(v)
	}
	fmt.Println("New Graph Edges", sum)

	// for source, edge := range newGraph.AdjacencyMap {
	// 	for destination := range edge {
	// 		if !graph.OldHasEdge(source, destination) {
	// 			fmt.Println(source, destination)
	// 			rwsetJsonSource := predictRwSets[source].ToJsonStruct().ToString()
	// 			rwsetJsonDestination := predictRwSets[destination].ToJsonStruct().ToString()
	// 			fmt.Println(rwsetJsonSource)
	// 			fmt.Println(rwsetJsonDestination)
	// 			return
	// 		}
	// 	}
	// }
